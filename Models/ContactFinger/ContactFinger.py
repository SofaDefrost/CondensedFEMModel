...
class Controller(BaseController):
    ...
    def get_actuators_state(self):
        cable = [float(cable.cableInitialLength.value)-float(cable.cableLength.value) for cable in self.list_actuators]
        contacts = []
    ...
    def apply_actions(self, value):
        nb_actuators = len(self.list_actuators)
        cable_value = value[:nb_actuators]
        contact_values = value[nb_actuators:]
    ...
    def get_contact(self):
        ...
        exit(1)
        """TODO: During the exploitation phase: I wanted to recover the force applied on the
        collision model and apply it as a state of my actuation to predict the matrix.
	    QUESTION: How do we recover this force?
        """

    # Usefull for testing in TestQP
    def get_effectors_state(self):
        return []

        # n_constraint = len(self.list_contacts) * 3
        # return [0 for _ in range(n_constraint)]

        # # Choose 1 contact point as effector
        # n_constraint = 3
        # return [0 for _ in range(n_constraint)]
    ...
    def get_effectors_positions(self):
        ...
        # return [contact.MechanicalObject.position.value for contact in self.list_contacts]

        # Choose 1 contact point as effector
        # return [self.list_contacts[0].MechanicalObject.position.value]
...
class Finger():
    ...
    def addEffectors(self, target, position=[[0.0, Const.Height/2, -3.0*Const.Length]]):
        ...
        effectors.addObject('ConstraintPoint', template='Vec3', indices = [i for i in range(len(TransformedPosEffectors))], effectorGoal=target.position.getLinkPath())
        effectors.addObject("BarycentricMapping", mapForces=False, mapMasses=False)
        return effectors

    def addContact(self, nb_points = 3, position_center= [0.0, Const.Height/2, -3.0*Const.Length], r = 1, use_collision = False):
        ...
        for i in range(nb_points):
            ...
            _contact.addObject("MechanicalObject", position=TransformedPosContacts, showObject = True, showObjectScale = 10)
            if use_collision:
                _contact.addObject('PointCollisionModel', group=0)
            _contact.addObject('PositionConstraint', template='Vec3', indices = [i for i in range(len(TransformedPosContacts))], valueType = valueType) # , effectorGoal=TransformedPosContacts
...
def createScene(rootNode, classConfig):
    ...
    finger.addContact(nb_points = config["nb_contact_points"], position_center=[0.0, Const.Height, -2.5*Const.Length], r = config["contact_radius"], use_collision = True)


    if not config["inverseMode"]:
        rootNode.addObject(Controller(name="Controller", constraint_solver = constraint_solver, constraint_solver_setter=constraint_solver_setter, list_actuators = finger.cables, list_contacts = finger.contacts, list_effectors = [], list_effectors_MO = []))
    return rootNode
