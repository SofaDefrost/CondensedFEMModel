...
def add_goal_node(parent, position = [0, 0, 0], showObject = True):
    ...
    goal_mo = goal.addObject('MechanicalObject', name='GoalMO', showObject=showObject, drawMode=1, showObjectScale=5,
...
class Finger():
    ...
    def addEffectors(self, target, position=[[0.0, Const.Height/2, -3.0*Const.Length]]):
        ...
        effectors.addObject("MechanicalObject", position=TransformedPosEffectors) #, showObject=True, drawMode=1, showObjectScale=3, showColor=[1, 0, 0, 1])
...
def createScene(rootNode, classConfig):
    ...
    radius = 40
    # finger1 = Finger(simulation, name = "Finger1", inverseMode=config["inverseMode"], is_force=config["is_force"], totalMass = 0.0, translation = [radius, 0, 0], rotation = [0, 0, 90])
    # finger2 = Finger(simulation, name = "Finger2", inverseMode=config["inverseMode"], is_force=config["is_force"], totalMass = 0.0, translation = [-radius, 0, 0], rotation = [0, 0, 90 + 180])
    ...
    finger2 = Finger(simulation, name = "Finger2", inverseMode=config["inverseMode"], is_force=config["is_force"], totalMass = 0.0, translation = [0, 2 * radius, 0], rotation = [0, 0, 180])
    ...
    if config["inverseMode"]:
        ...
        finger1.addEffectors(goal_mo, position = [[0.0, Const.Height/2, -3.0*Const.Length]])
        finger2.addEffectors(goal_mo, position = [[0.0, Const.Height/2, -3.0*Const.Length]])
    else:
        goal_mo = add_goal_node(rootNode, config["goalPos"], showObject=False)
        effector1 = finger1.addEffectors(goal_mo, position = [[0.0, Const.Height/2, -3.0*Const.Length]])
        effector2 = finger2.addEffectors(goal_mo, position = [[0.0, Const.Height/2, -3.0*Const.Length]])
        rootNode.addObject(Controller(name="Controller", constraint_solver = constraint_solver, constraint_solver_setter = constraint_solver_setter, list_actuators = finger1.cables + finger2.cables, list_effectors = [rootNode.Simulation.Finger1.Effectors, rootNode.Simulation.Finger2.Effectors],
        list_effectors_MO = [rootNode.Simulation.Finger1.Effectors.MechanicalObject, rootNode.Simulation.Finger2.Effectors.MechanicalObject]))
    ...
    # Add a cube for visualization purpose
    cube_obj = MeshPath + "cube.obj"
    cube = finger1.node.Effectors.addChild("Cube")
    finger2.node.Effectors.addChild(cube)
    cube.addObject("MechanicalObject", name = "dofs")
    cube.addObject('SubsetMultiMapping', template='Vec3,Vec3',
                             name='CtE' ,
                             input=[finger1.node.Effectors.MechanicalObject.getLinkPath(), finger2.node.Effectors.MechanicalObject.getLinkPath()],
                             output='@./dofs',
                             indexPairs=[[0,0], [1,0]])

    colliCube = cube.addChild("collis")
    colliCube.addObject('MeshTopology', position="@../dofs", edges = [0,1])
