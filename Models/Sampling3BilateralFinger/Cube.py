...
def addRigidObject(node, filename, collisionFilename=None, position=[0,0,0,0,0,0,1], scale=[1,1,1], textureFilename='', color=[1,1,1], density=0.002, name='Object', withSolver=True, collisionGroup = 0, withCollision=True):
    ...
    # object.addObject("FixedConstraint")
...
class Cube():
    ...
    def onEnd(self, rootNode, collisionGroup = 1, withSolver = True):
        print(">>  Init Cube")
        self.cube = addRigidObject(rootNode,filename=meshPath+'cube.obj',name= self.name,scale=self.scale, position=self.init_pos+[0, 0, 0, 1], density=self.density, withSolver= withSolver, collisionGroup = collisionGroup)
        # self.cube = addRigidFEMObject(rootNode,filename=meshPath+'cube.vtk',name= self.name,scale=self.scale, position=self.init_pos+[0, 0, 0, 1], density=self.density, collisionGroup = collisionGroup, withSolver = withSolver)
    ...
    def addContact(self, pos_contacts):
        ...
        for i in range(len(pos_contacts)):
            ...
            # _contact.addObject('ConstraibtPoint', template='Vec3', indices = [0], effectorGoal=pos_contacts[i], valueType = "force", imposedValue = [0,0,0])
