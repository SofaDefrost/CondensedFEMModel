...
def createScene(rootNode, classConfig):
    ...
    #Fingers
    dist_y, dist_x = 70, 30
    finger = Finger(simulation, name = "Finger1", inverseMode=config["inverseMode"],  is_force=config["is_force"], totalMass = 0.0, translation = [0, 0, 0], rotation = [0, 0, 0])
    ...
    pos_contacts = finger.addContact(nb_points = config["nb_contact_points"], position_center=[0.0, Const.Height, -2.5*Const.Length], r = config["contact_radius"], use_collision = False, group=0)
    ...
    cube.addContact(pos_contacts)


    if not config["inverseMode"]:
        rootNode.addObject(Controller(name="Controller", config = classConfig, constraint_solver = constraint_solver, constraint_solver_setter=constraint_solver_setter, list_actuators = finger.cables, list_contacts = finger.contacts, list_effectors = list_effectors, list_effectors_MO = list_effectors_MO, list_cube_contacts = cube.contacts))
