...
class Controller(BaseController):
    ...
    def compute_B_from_dfree_c(self, X_cube, lambda_c, D, Jc, Wcc, dfree_c, xc_cube):
        ...
        return inter_2 + np.matmul(np.matmul(D, Jc.T), lambda_c)

    def compute_QP_matrices(self, A, b, delta_free_a, Waa, Wca, lambda_a,  use_epsilon=False):

        if use_epsilon:
            epsilon = 0.0001 * np.linalg.norm(np.dot(np.transpose(Wca), Wca), ord=1) / np.linalg.norm(Waa, ord=1)
        else:
            epsilon = 0.0

        H = 2 * np.matmul(A.T, A) + epsilon * (Waa).astype('double')
        g = 2 * np.matmul(A.T, b)
        C = Waa

        ### Actuation constraints
        l = np.zeros(lambda_a.shape) + 0.0
        u = np.zeros(lambda_a.shape) + np.infty
    ...
    def update_object_position(self, X):
        X_t = X[:3].tolist() + self.euler_to_quaternion(X[3], X[4], X[5])
        self.cube.MechanicalObject.position.value = [X_t]
    ...
    def compute_W_from_simulation(self, n_finger, n_constraint, n_act_constraint, W_t):
        ...
        for i in range(n_finger):
            Waa_i = W_t[i * n_constraint: i * n_constraint + n_act_constraint,
    ...
    def onSolveConstraintSystemEndEvent(self, event):
        ...
        if self.step < self.waiting_time:
            ...
            if MASK_ROTATION:
                ...
                H, g, C, l, u, lb, ub = self.compute_QP_matrices(masked_A, masked_b, delta_free_a, Waa, Wca, self.lambda_a)
            else:
                H, g, C, l, u, lb, ub = self.compute_QP_matrices(A,  b, delta_free_a, Waa, Wca, self.lambda_a)

            # Solve QP
            new_lambda_a = solve_QP(self.solver, H, g, C, l, u, lb, ub, is_init=(self.step != self.waiting_time))

            new_X_cube = self.compute_X(A, B, new_lambda_a)
            new_lambda_c = self.compute_lambda_c_from_dfree_c(X_cube, new_X_cube, Wcc, Wca, Jc, new_lambda_a, xc_cube, dfree_c)
            ...
            new_X_cube = self.compute_X(A, B, self.lambda_a)  # Recompute object position with correction
            self.update_object_position(new_X_cube)

            assert len(self.list_actuators) == self.lambda_a.shape[0]
            for i, actuator in enumerate(self.list_actuators):
                actuator.value.value = [self.lambda_a[i]]

            for i, point_finger in enumerate(self.list_contacts):
                point_cube = self.list_cube_contacts[i]
            ...
            lambda_constraint_vector = copy.deepcopy(self.constraint_solver.lambda_force())
            ...
            for robot_id in range(self.config.nb_robot):
                ...
                for i in range(n_contact_constraint):
                    lambda_constraint_vector[robot_id * self.n_constraint + self.n_act_constraint + i] = + \
                    self.lambda_c[robot_id * n_contact_constraint + i]
            # Object Contacts
            ...
            self.constraint_solver.set_lambda_force(lambda_constraint_vector)
...
class Finger():
    ...
    def addContact(self, nb_points = 3, position_center= [0.0, Const.Height/2, -3.0*Const.Length], r = 1, use_collision = False, group = 0):
        ...
        for i in range(nb_points):
            ...
            _contact.addObject('ConstraintPoint', template='Vec3', indices = [0], valueType = valueType)
...
def createScene(rootNode, classConfig):
    classConfig.set_action_type(is_force = True)
    ...
    # rootNode.addObject('RuleBasedContactManager', responseParams="mu="+str(1.2), name='Response',
    #                        response='FrictionContactConstraint')
    # rootNode.addObject('LocalMinDistance', alarmDistance=20, contactDistance=1, angleCone=0.01)


    rootNode.gravity.value = [0., 0, -9810.] # [0., 0, -9810.]
    rootNode.dt.value = 0.01

    simulation = rootNode.addChild("Simulation")

    simulation.addObject('EulerImplicitSolver', name='odesolver', firstOrder=1, rayleighMass=0.1,  rayleighStiffness=0.1)
    ...
    # goal_mo1 = add_goal_node(rootNode, name = "Goal1", position = config["goalPos"])
    # goal_mo2 = add_goal_node(rootNode, name = "Goal2", position = config["goalPos"])
    # goal_mo3 = add_goal_node(rootNode, name = "Goal3", position = config["goalPos"])

    # effectors1 = finger1.addEffectors(goal_mo1, position = [[0.0, Const.Height/2, -3.0*Const.Length]])
    # effectors2 = finger2.addEffectors(goal_mo2, position = [[0.0, Const.Height/2, -3.0*Const.Length]])
    # effectors3 = finger3.addEffectors(goal_mo3, position = [[0.0, Const.Height/2, -3.0*Const.Length]])
    # list_effectors = [effectors1.ConstraintPoint, effectors2.ConstraintPoint, effectors3.ConstraintPoint]
    # list_effectors_MO = [effectors1.MechanicalObject, effectors2.MechanicalObject, effectors3.MechanicalObject]
    list_effectors = []
    list_effectors_MO = []
    ...
    #Cube
    cube_config = {"init_pos": [0, dist_y/2, -3.0*Const.Length+25], "scale": [dist_y/2, dist_y/8+5, dist_y/8+5], "density": 0}
    cube = Cube(name = "Cube", cube_config=cube_config)
    cube.onEnd(rootNode, collisionGroup=1)
    cube.addContact(pos_contacts_1 + pos_contacts_2 + pos_contacts_3)

    # WARNING: the floor is lava
    #floor_config = {"init_pos": [20, 20, -3.0*Const.Length-5], "scale": [100.0,100,0.2], "density": 1e-12}
    #floor = Cube(name = "Floor", cube_config=floor_config)
    #floor.onEnd(rootNode, collisionGroup=2)
    #floor.cube.addObject('FixedConstraint', indices=0)

    if not config["inverseMode"]:
        rootNode.addObject(Controller(name="Controller", cube = cube.cube, config = classConfig, constraint_solver = constraint_solver, constraint_solver_setter=constraint_solver_setter, list_actuators = finger1.cables + finger2.cables + finger3.cables, list_contacts = finger1.contacts + finger2.contacts + finger3.contacts, list_effectors = list_effectors, list_effectors_MO = list_effectors_MO, list_cube_contacts = cube.contacts))
