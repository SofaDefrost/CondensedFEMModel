...
class Controller(BaseController):
    ...
    def get_effectors_state_calibration(self):
        effector_state = []
        for effector_MO in self.list_effectors_MO:
            pos = effector_MO.position.value[0]
            rest_pos = effector_MO.rest_position.value[0]
...
def createScene(rootNode, config):
    ...
    model.addObject('MeshVTKLoader', name='loader',
                    filename = MeshPath + 'Finger_Volumetric.vtk')
    ...
    modelVisu.addObject('MeshSTLLoader', name="loader",
                        filename =MeshPath + 'Finger_Surface.stl')
    ...
    goal_mo = goal.addObject('MechanicalObject', name='GoalMO', showObject=True, drawMode=1, showObjectScale=3,
                             showColor=[237/255, 112/255, 20/255, 255/255], position=scene_config["goalPos"])
    # Effector
    effectors = model.addChild("Effectors")
    effector_pos = [[0.0, config.Height/2, -3.0*config.Length]]
    effectors.addObject("MechanicalObject", position=effector_pos, showObject=True, drawMode=1, showObjectScale=3, showColor=[0, 1, 0, 1])
