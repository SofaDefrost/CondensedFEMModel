# -*- coding: utf-8 -*-
"""PneumaticTrunk.py: create scene of the PneumaticTrunk
...
import json
import numpy as np
from SofaController import EmbeddedController
...
def create_pneumatic_cavity(points, mesh, parent, num_module, num_cavity, value_type='pressure', init_pressure=0,
                            min_pressure=0, max_pressure=200, inverse_mode=False):
    ...
    if inverse_mode:
        CavityNode.addObject('SurfacePressureActuator', name='SPC', template='Vec3d',
                             triangles='@chambreAMesh' + str(num_module + 1) + '.triangles', valueType=value_type,
                             minPressure=min_pressure, maxPressure=max_pressure)
    else:
        CavityNode.addObject('SurfacePressureConstraint', name='SPC',
                             triangles='@chambreAMesh' + str(num_module + 1) + '.triangles',
...
def add_goal_node(parent, use_3_effectors, position=[0, 0, 0], showObject=True):
    ...
    if use_3_effectors:
        ...
        for i in range(len(position_goal)):
            ...
            goal_mo_i = goal_i.addObject('MechanicalObject', name='GoalMO', showObject=showObject, drawMode=1,
                                         showObjectScale=1, showColor=[1, 0, 0, 1], position=position_goal[i])
            goal_i.addObject("RigidMapping", mapForces=False, mapMasses=False)
            goal_mo.append(goal_mo_i)

    else:
        goal_mo.append(
            goal.addObject('MechanicalObject', name='GoalMO', showObject=showObject, drawMode=1, showObjectScale=2,
                           showColor=[1, 0, 0, 1], position=position))
...
class Stiff_Flop():
    ...
    def __init__(self, name="Stiff_Flop", nb_slices=16, init_pressure=0, min_pressure=0, max_pressure=200,
                 is_force=True, inverseMode=False):
        """
        Parameters:
        -----------
    ...
    def create_and_init_box(self, parent, positions, module_number):
        ...
        parent.addObject('BoxROI', name="DISPLAY_boxROI_III_K" + str(module_number + 1), template="Vec3d",
                         orientedBox=[III_K_x, III_K_y, self.h_module * module_number, III_K_x2, III_K_y2,
                                      self.h_module * module_number,
                                      III_K_x3, III_K_y3, self.h_module * module_number, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)

        parent.addObject('BoxROI', name="DISPLAY_boxROI_III_H" + str(module_number + 1), template="Vec3d",
                         orientedBox=[DA3x - 0.6, DA3y - 0.6, self.h_module * module_number, E3x + 1.5, E3y + 0.5, 0,
                                      Ex + 1.5,
                                      Ey + 0.5, self.h_module * module_number, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)

        parent.addObject('BoxROI', name="DISPLAY_boxROI_II_K" + str(module_number + 1), template="Vec3d",
                         orientedBox=[II_K_x, II_K_y, self.h_module * module_number, II_K_x2, II_K_y2,
                                      self.h_module * module_number, II_K_x3,
                                      II_K_y3, self.h_module * module_number, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)

        parent.addObject('BoxROI', name="DISPLAY_boxROI_II_H" + str(module_number + 1), template="Vec3d",
                         orientedBox=[II_H_x, II_H_y, self.h_module * module_number, II_H_x2, II_H_y2,
                                      self.h_module * module_number, II_H_x3,
                                      II_H_y3, self.h_module * module_number, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)

        parent.addObject('BoxROI', name="DISPLAY_boxROI_I_K" + str(module_number + 1), template="Vec3d",
                         orientedBox=[I_K_x, I_K_y, self.h_module * module_number, I_K_x2, I_K_y2,
                                      self.h_module * module_number, I_K_x3,
        ...
        parent.addObject('BoxROI', name="DISPLAY_boxROI_I_H" + str(module_number + 1), template="Vec3d",
                         orientedBox=[I_H_x, I_H_y, self.h_module * module_number, I_H_x2, I_H_y2,
                                      self.h_module * module_number, I_H_x3,
        ...
        parent.addObject('BoxROI', name="boxROI_III_K" + str(module_number + 1), template="Vec3d",
                         orientedBox=[self.h_module * module_number, III_K_y, III_K_x, self.h_module * module_number,
                                      III_K_y2, III_K_x2,
                                      self.h_module * module_number, III_K_y3, III_K_x3, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True,
                         drawQuads=False)

        parent.addObject('BoxROI', name="boxROI_III_H" + str(module_number + 1), template="Vec3d",
                         orientedBox=[self.h_module * module_number, DA3y - 0.6, DA3x - 0.6,
                                      self.h_module * module_number, E3y + 0.5,
        ...
        parent.addObject('BoxROI', name="boxROI_II_K" + str(module_number + 1), template="Vec3d",
                         orientedBox=[self.h_module * module_number, II_K_y, II_K_x, self.h_module * module_number,
                                      II_K_y2, II_K_x2,
                                      self.h_module * module_number, II_K_y3, II_K_x3, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)

        parent.addObject('BoxROI', name="boxROI_II_H" + str(module_number + 1), template="Vec3d",
                         orientedBox=[self.h_module * module_number, II_H_y, II_H_x, self.h_module * module_number,
                                      II_H_y2, II_H_x2,
                                      self.h_module * module_number, II_H_y3, II_H_x3, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)

        parent.addObject('BoxROI', name="boxROI_I_K" + str(module_number + 1), template="Vec3d",
                         orientedBox=[self.h_module * module_number, I_K_y, I_K_x, self.h_module * module_number,
                                      I_K_y2, I_K_x2,
                                      self.h_module * module_number, I_K_y3, I_K_x3, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)

        parent.addObject('BoxROI', name="boxROI_I_H" + str(module_number + 1), template="Vec3d",
                         orientedBox=[self.h_module * module_number, I_H_y, I_H_x, self.h_module * module_number,
                                      I_H_y2, I_H_x2,
                                      self.h_module * module_number, I_H_y3, I_H_x3, self.h_module * 2],
                         drawBoxes=display_flag,
                         strict=True)
        ...
        object_list_box = ['boxROI_III_K' + str(module_number + 1), 'boxROI_III_H' + str(module_number + 1),
                           'boxROI_II_K' + str(module_number + 1),
                           'boxROI_II_H' + str(module_number + 1), 'boxROI_I_K' + str(module_number + 1),
                           'boxROI_I_H' + str(module_number + 1)]
    ...
    def create_module_extrude(self, parent, num_module, module_model_path):
        ...
        module.addObject('MeshOBJLoader', name=object_list[0], filename=module_model_path,
                         translation=[self.h_module * num_module, 0, 0], rotation=[0, 0, 90])
    ...
    def create_cavity(self, module, num_module):
        ...
        """
        self.create_and_init_box(parent=module,
                                 positions=get_cavity_box_position(r_disk=self.r_disk_box, l_box=self.l_box),
                                 module_number=num_module)
        ...
        for j in range(self.nb_cavity):
            if os.path.isfile(file):
                ...
                [new_points_K, triangles_K, _] = cylinder_mesh_from_ROI(points=Boite_K.pointsInROI.value,
                                                                        quads=Boite_K.quadInROI.value,
                                                                        indices=Boite_K.indices.value)
                [new_points_H, triangles_H, _] = cylinder_mesh_from_ROI(points=Boite_H.pointsInROI.value,
                                                                        quads=Boite_H.quadInROI.value,
                                                                        indices=Boite_H.indices.value)

                nb_tri = len(new_points_K)
                for d in range(len(triangles_H)):
                    triangles_H[d] = [triangles_H[d][0] + nb_tri, triangles_H[d][1] + nb_tri,
                                      triangles_H[d][2] + nb_tri]
            ...
            CavityNode = create_pneumatic_cavity(points=new_points2, mesh=triangles, parent=module,
                                                 num_module=num_module, num_cavity=j, value_type=self.value_type,
                                                 init_pressure=self.init_pressure, min_pressure=self.min_pressure,
                                                 max_pressure=self.max_pressure, inverse_mode=self.inverse_mode)
            ...
            CavityNode.addObject('AdaptiveBeamMapping', interpolation='@../../BeamInterpolation', input='@../../DOFs',
                                 output='@./chambreA' + str(num_module + 1))  # classic
    ...
    def createRobot(self, parent):
        ...
        for num_module in range(self.nb_module):
            ...
            parent.addObject('MechanicalMatrixMapper', template='Rigid3d,Rigid3d', object1='@DOFs', object2='@DOFs',
                             nodeToParse='@./stiff_flop' + str(num_module + 1))
    ...
    def addEffectors(self, target, use_3_effectors, showObject=True):
        ...
        if use_3_effectors:
            r, ang = 4, 2 * np.pi / 3
            position_goal = [[position[0], position[1] + r * np.sin(i * ang + np.pi / 6),
                              position[2] + r * np.cos(i * ang + np.pi / 6)] for i in
                             range(3)]
        ...
        if use_3_effectors:
            for i in range(3):
                ...
                effectors_i_MO = effectors_i.addObject("MechanicalObject", position=position_goal[i],
                                                       showObject=showObject, drawMode=1, showObjectScale=1,
                                                       showColor=[0, 1, 0, 1])
                effector_i_constraint = effectors_i.addObject('PositionConstraint', template='Vec3',
                                                              indices=[i for i in range(len(position_goal[i]))],
                                                              effectorGoal=target[i].position.getLinkPath(),
                                                              valueType="force", imposedValue=[0, 0, 0])
            ...
            effectors_i_MO = effectors_i.addObject("MechanicalObject", position=[position], showObject=showObject,
                                                   drawMode=1, showObjectScale=1, showColor=[0, 1, 0, 1])
            effector_i_constraint = effectors_i.addObject('PositionConstraint', template='Vec3', indices=[0],
                                                          effectorGoal=target[0].position.getLinkPath(),
                                                          valueType="force", imposedValue=[0, 0, 0])
...
def PneumaticTrunkScene(rootNode, config, nb_slices=16, dt=0.1):
    ...
    rootNode.addObject('RequiredPlugin', name='Sofa.Component.Mapping.MappedMatrix')
    ...
    rigidFramesNode.addObject('EulerImplicitSolver', firstOrder='0', vdamping=0, rayleighStiffness='0.3',
                              rayleighMass='0.1')
    ...
    rigidFramesNode.addObject('RegularGridTopology', name='meshLinesCombined', nx=PneumaticTrunk.nb_beam, ny='1',
                              nz='1', xmax=PneumaticTrunk.h_module * PneumaticTrunk.nb_module, xmin='0.0', ymin='0',
                              ymax='0', zmin='0', zmax='0')
    rigidFramesNode.addObject('MechanicalObject', name='DOFs', template='Rigid3d', showObject='1', showObjectScale='1',
                              rotation=[0, 0, 0], translation=[0, 0, 0])  # -90 on y
    rigidFramesNode.addObject('BeamInterpolation', name='BeamInterpolation', printLog='0', dofsAndBeamsAligned='true',
                              straight='1', crossSectionShape='circular',
                              radius=PneumaticTrunk.r_cavity + PneumaticTrunk.r_disk_chamber)
    rigidFramesNode.addObject('RestShapeSpringsForceField', name='anchor', points='0', stiffness='1e12',
                              angularStiffness='1e12')


    goal_mo = add_goal_node(rootNode, use_3_effectors=config["use3Effectors"], position=config["goalPos"],  showObject=True)
    PneumaticTrunk.createRobot(parent=rigidFramesNode)
    effector_MO, _ = PneumaticTrunk.addEffectors(target=goal_mo, use_3_effectors=config["use3Effectors"])

    rootNode.addObject(EmbeddedController(name="Controller", root=rootNode, list_actuators=PneumaticTrunk.cavities, goalMO = goal_mo[0], effectorMO = effector_MO[0]))

    return rootNode

def createScene(rootNode):
    config = {"goalPos": [110, 0, 0],
                           "use3Effectors": False,
                           "actuatorStateType": "volume",
                           "name": "PneumaticTrunk",
                           "is_force": True}
    PneumaticTrunkScene(rootNode, config, nb_slices=10)
    return rootNode
